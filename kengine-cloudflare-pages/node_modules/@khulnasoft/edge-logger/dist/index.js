...
var KengineLogger = class {
	...
	constructor(args) {
		...
		this.kengineUrl = args.kengineUrl ?? "https://events.khulnasoft.com/v1";
		if (args.requestId) {
	...
	async flush({ skipIfInProgress = false } = {}) {
		...
		const doFlush = async () => {
			...
			try {
				const res = await fetch(`${this.kengineUrl}/${this.dataset}`, {
					method: "POST",
					headers: {
				...
				if (res.ok) {
					...
					console.log(
						`Kengine failed to ingest logs: ${res.status} ${
							res.statusText
						} ${await res.text()}`,
					);
				}
