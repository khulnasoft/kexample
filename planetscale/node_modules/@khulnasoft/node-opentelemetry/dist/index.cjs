...
__export(src_exports, {
  ...
  KengineSDK: () => KengineSDK,
  StripePlugin: () => StripePlugin,
  VercelPlugin: () => VercelPlugin,
  trpcTracingMiddleware: () => tracing
});
...
var VercelDetector = class {
  detect(_config) {
    if (!process.env.VERCEL) {
      return import_resources.Resource.empty();
    }
    ...
    return new import_resources.Resource(attributes);
  }
};
...
var KengineSDK = class {
  ...
  start() {
    ...
    const provider = new import_sdk_trace_node.NodeTracerProvider({
      resource: (0, import_resources2.detectResourcesSync)({
        detectors: [import_resource_detector_aws.awsEcsDetector, import_resource_detector_aws.awsEc2Detector, import_resource_detector_aws.awsLambdaDetector, new VercelDetector()]
      }),
      forceFlushTimeoutMillis: 500
    });
    if ((0, import_fs.existsSync)("/opt/extensions/kengine")) {
...
function tracing(options) {
  const tracer = import_api.trace.getTracer("@khulnasoft/trpc");
  options = options || {};
...
// src/utils/captureRequestBody.ts
var import_shimmer = __toESM(require("shimmer"), 1);
...
function patchWriteOrEndReq(original) {
  return function() {
...
function captureRequestBody(request) {
  return new Promise((resolve) => {
    import_shimmer.default.wrap(request, "write", patchWriteOrEndReq);
    import_shimmer.default.wrap(request, "end", patchWriteOrEndReq);
...
function _betterHttpInstrumentation(options = {}) {
  ...
  return {
    requestHook(span, request) {
      if (request instanceof import_http.ClientRequest) {
        const plugin2 = options.plugins.find((plugin3) => plugin3.shouldParseRequest && plugin3.shouldParseRequest(request));
      ...
      if (request instanceof import_http.IncomingMessage) {
        ...
        if (plugin2.parseIncommingMessage) {
          const attributes = plugin2.parseIncommingMessage(request);
...
var BetterHttpInstrumentation = class extends import_instrumentation_http.HttpInstrumentation {
  constructor(options = {}) {
    super({
...
var StripePlugin = class extends HttpPlugin {
  ...
  name = "stripe";
  shouldParseRequest(request) {
    if (request instanceof import_http2.ClientRequest && request.host?.includes("api.stripe.com")) {
      return true;
    }
    return false;
  }
...
var VercelPlugin = class extends HttpPlugin {
  name = "vercel";
  shouldParseRequest(request) {
    if (request instanceof import_http3.IncomingMessage && request.headers["x-vercel-id"]) {
      return true;
    }
    return false;
  }
  parseIncommingMessage(request) {
