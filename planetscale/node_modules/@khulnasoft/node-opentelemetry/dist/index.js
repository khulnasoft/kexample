...
function tracing(options) {
  const tracer = trace.getTracer("@khulnasoft/trpc");
  options = options || {};
...
import { ClientRequest as ClientRequest2 } from "http";
...
var StripePlugin = class extends HttpPlugin {
  ...
  name = "stripe";
  shouldParseRequest(request) {
    if (request instanceof ClientRequest2 && request.host?.includes("api.stripe.com")) {
      return true;
    }
    return false;
  }
...
import { IncomingMessage as IncomingMessage3 } from "http";
var VercelPlugin = class extends HttpPlugin {
  name = "vercel";
  shouldParseRequest(request) {
    if (request instanceof IncomingMessage3 && request.headers["x-vercel-id"]) {
      return true;
    }
    return false;
  }
...
export {
  ...
  StripePlugin,
  VercelPlugin,
  tracing as trpcTracingMiddleware
};
