import { CloudWatch, Kinesis, SSM } from 'aws-sdk'
...
async function checkScalingNeeds(
  input: Input,
  currentShardCount: number
): Promise<ScalingNeed> {
  const cw = new CloudWatch()
  const alarmStates = await cw
    .describeAlarms({
      AlarmNames: [input.scaleUpAlarmName, input.scaleDownAlarmName],
    })
    .promise()
  logger.info('Alarm states', { alarmStates })
  const scaleUpAlarm = alarmStates.MetricAlarms?.find(
    (a) => a.AlarmName === input.scaleUpAlarmName
  )
  const scaleDownAlarm = alarmStates.MetricAlarms?.find(
    (a) => a.AlarmName === input.scaleDownAlarmName
  )
